...
function getUnreadMessageCount() {
    const data = getData();
    return (data.contactMessages || []).filter(msg => msg.status === 'unread').length;
}
...
function loadMessagesList() {
    ...
    if (messagesList) {
        ...
        messagesList.innerHTML = data.contactMessages.map(msg => `
            <div class="contact-message-item ${msg.status === 'unread' ? 'unread-message' : ''}">
                <div class="contact-message-header">
                    <div>
                        <h4>${msg.subject}</h4>
                        <p>${msg.name} • ${msg.email} • ${msg.phone}</p>
                    </div>
...
function markMessageAsRead(id) {
    const data = getData();
    const message = (data.contactMessages || []).find(msg => msg.id === id);
    if (message) {
        message.status = 'read';
        saveData(data);
        updateMessagesBadge();
...
function markAllAsRead() {
    const data = getData();
    (data.contactMessages || []).forEach(msg => {
        msg.status = 'read';
    });
    saveData(data);
    updateMessagesBadge();
...
function addAdmin(event) {
    ...
    // If role is super_admin, give all permissions
    if (role === 'super_admin') {
        ...
        // Admin has most permissions except maybe admins
        permissions.hospital_info = true;
        permissions.doctors = true;
        permissions.timings = true;
        permissions.payments = true;
            ...
            <div class="admin-info">
                ...
                <div style="margin-top: 8px; font-size: 0.85rem; color: #666;">
                    ...
                    <div style="margin-top: 4px;">
                        ${admin.permissions.hospital_info ? '✓ Hospital Info' : ''} ${admin.permissions.doctors ? '✓ Doctors' : ''} ${admin.permissions.timings ? '✓ Timings' : ''}<br/>
                        ${admin.permissions.payments ? '✓ Payments' : ''} ${admin.permissions.requests ? '✓ Requests' : ''} ${admin.permissions.admins ? '✓ Manage Admins' : ''}
                    </div>
                </div>
            </div>
...
function loadSettingsPage() {
    ...
    // Load admin section only if user has permission
    ...
    // Load contact messages tab
    updateMessagesBadge();
    loadMessagesList();

    // Check and show tabs based on permissions
...
function loadRequestsList() {
    ...
    const pendingRequests = data.appointmentRequests.filter(r => r.status === 'pending');
    ...
    requestsList.innerHTML = pendingRequests.map(req => `
        <div class="request-item">
            ...
            <div class="request-actions">
                <button class="btn btn-success" onclick="approveRequest('${req.id}')">Approve & Send Payment Link</button>
...
function approveRequest(requestId) {
    ...
    const data = getData();
    const request = data.appointmentRequests.find(r => r.id === requestId);
...
function deleteMessage(id) {
    ...
    const data = getData();
    data.contactMessages = (data.contactMessages || []).filter(msg => msg.id !== id);
    saveData(data);
    updateMessagesBadge();
    loadMessagesList();
    loadDashboardData();
